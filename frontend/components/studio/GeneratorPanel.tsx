...
const GeneratorPanel = memo(function GeneratorPanel({
  ...
  // --- LOCAL PROMPT STATE (Fixes Typing Lag) ---
  const [localPrompt, setLocalPrompt] = useState("");

  // Sync local prompt when switching between shots
  useEffect(() => {
    setLocalPrompt(activeShot?.prompt || "");
  }, [activeShot?.id]);

  // Debounce saving the prompt to the global state/disk
  useEffect(() => {
    const handler = setTimeout(() => {
      if (activeShot && localPrompt !== activeShot.prompt) {
        updateActiveShot({ prompt: localPrompt });
      }
    }, 400); // Waits 400ms after you stop typing to save
    return () => clearTimeout(handler);
  }, [localPrompt, activeShot, updateActiveShot]);
  ...
  useEffect(() => {
    ...
    const stopStatus = EventsOn("comfy:status", (s: string) => setProgressStatus(s));
    return () => {
      stopProgress();
      stopStatus();
    };
  ...
  const handleAppendPrompt = (text: string) => {
    let current = localPrompt.trim();
    if (current.length > 0 && !current.endsWith(",")) {
      current += ", ";
    }
    const newPrompt = current + text;
    setLocalPrompt(newPrompt);
    updateActiveShot({ prompt: newPrompt }); // Force immediate sync
  };
    ...
    try {
      ...
      if (selectedWorkflow === "wan-2.2-cloud") {
        ...
        updatedShot = await RenderRemoteShot(
          ...
          scene.id,
          activeShot.id,
          localPrompt, // Used localPrompt here so it's always up to date
          activeShot.sourceImage,
          remoteUrl,
          activeShot.audioPath || "",
          activeShot.audioStart || 0,
          activeShot.audioDuration || 0,
        );
      } else {
        updatedShot = await RenderShot(
          project.id,
          scene.id,
          { ...activeShot, prompt: localPrompt }, // Used localPrompt here
          selectedWorkflow,
        );
      }
  ...
  if (!activeShot) return <div className="p-10 text-xs text-zinc-500">Select a shot</div>;
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <span className="text-[9px] font-bold uppercase tracking-wider">
              {displayStatus === "success" && (isCloud ? "Cloud Ready" : "System Ready")}
              {displayStatus === "error" && (isCloud ? "Cloud Offline" : "System Offline")}
              {(displayStatus === "testing" || displayStatus === "idle") && (isCloud ? "Connecting Cloud..." : "Connecting...")}
            </span>
          </div>
        </div>
        ...
        <div>
          ...
          >
            {activeShot.previewBase64 ? (
              <img src={activeShot.previewBase64} className="w-full h-full object-cover" />
            ) : (
              <>
                <ImageIcon size={24} />
            ...
            {activeShot.audioPath && (
              <div className="space-y-2">
                <div className="flex items-center justify-between text-[10px] text-zinc-500 px-1">
                  ...
                  <button onClick={handleClearAudio} className="hover:text-red-500 flex items-center gap-1">
                    <X size={10} /> Clear
                  </button>
                </div>
                <TrimmableWaveform
                  ...
                  onTrimChange={(start, duration) => {
                    updateActiveShot({ audioStart: start, audioDuration: duration });
                  }}
                />
              </div>
            )}
        ...
        <div className="space-y-4">
          ...
          {selectedWorkflow.toLowerCase().includes("wan2") && (
            <div className="flex items-center justify-between text-xs animate-in fade-in slide-in-from-top-1 mt-2">
              <span className="text-zinc-500 font-medium">Generation Length</span>
              <span className="font-mono font-bold text-[#D2FF44] tracking-wide">5 Seconds</span>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) => updateActiveShot({ seed: parseInt(e.target.value) })}
              />
              <button
                onClick={() => updateActiveShot({ seed: Math.floor(Math.random() * 1000000) })}
          ...
          <div className="mt-4">
            {isRendering ? (
              ...
              >
                ...
                {isReady ? "Render Shot" : isCloud ? "Cloud Offline" : "System Offline"}
              </button>
            )}
          </div>
        </div>
      ...
      <AssetPickerModal isOpen={showModal} onClose={() => setShowModal(false)} type={modalType} project={project} onSelect={handleAssetSelected} />
      <CameraModal isOpen={showCamera} onClose={() => setShowCamera(false)} onSelect={handleAppendPrompt} />
      <PromptWizardModal isOpen={showWizard} onClose={() => setShowWizard(false)} onSelect={handleAppendPrompt} />
    </>
  );
});
